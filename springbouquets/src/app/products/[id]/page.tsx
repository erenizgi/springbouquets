...
const ProductPage = ({params}) => {
    ...
    return <div className={"flex flex-col justify-center items-center"}>
        <Login loggedIn={loggedIn} setLoggedIn={setLoggedIn} user={user} customStyle={{position: "relative", zIndex: "100"}}
               isAdmin={true} setPopUp={() => {
        }}></Login>

        <div style={{backgroundImage: `url(${bride.src})`}}
             className={"bg-cover bg-center flex justify-center items-center flex-col gap-64"}>
            <div
                ...
                <div className={"pt-8 w-[50%] flex items-start justify- center flex-col"}>
                    ...
                    <div className={"flex gap-8 py-2 border p-4 w-fit"}>
                        <button style={{color: quantity === 1 ? "rgba(0,0,0,.5)" : "black"}} disabled={quantity === 1} onClick={() => {
                            if (quantity > 1) {
                    ...
                    <h3 className={"transition duration-250 hover:scale-105 cursor-pointer w-full border p-2 text-center "}>Add to Cart</h3>
                    <div className={"mt-16"}>
                        <h3>PRODUCT INFO</h3>
